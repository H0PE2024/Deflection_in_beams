...
syms a;
syms b; %for unknown moments at ends
b = 0;
addPrompt = ' :  '; %For changing characters added in front of prompts for taking input
e = 0.05; %For chaning the least value of x while plotting graphs
...
%These supports will be fixed joints (different from end points)
disp('All supports are fixed joints');
...
disp('Specifying coordinates of points, that is, fixed joints');
disp('Origin is the beginning of the beam');

for i=1:ns
    if i==1
        prompt = 'Enter coordinate of support (Fixed joint)';
    else
